...
export function createUserSkillsDal(db: Database) {
  return {
    ...
    async get(userId: string, id: string) {
      requireUserId(userId);
      const rows = await db.select().from(userSkills)
        .where(and(eq(userSkills.userId, userId), eq(userSkills.id, id))).limit(1);
    ...
    async update(userId: string, id: string, data: Partial<{
      ...
      requireUserId(userId);
      const rows = await db.update(userSkills).set({ ...data, updatedAt: new Date() })
        .where(and(eq(userSkills.userId, userId), eq(userSkills.id, id))).returning();
      if (!rows[0]) throw new OwnershipError('user_skill', id);
      return rows[0];
    },
    ...
    async delete(userId: string, id: string) {
      requireUserId(userId);
      const rows = await db.delete(userSkills)
        .where(and(eq(userSkills.userId, userId), eq(userSkills.id, id))).returning();
      if (!rows[0]) throw new OwnershipError('user_skill', id);
      return rows[0];
    },
