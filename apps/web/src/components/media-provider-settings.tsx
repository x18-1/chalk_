...
import { mediaApi, type MediaCapability, type MediaProvider, type MediaProviders } from "../api";
...
export function MediaProviderSettings({ capability }: { capability: MediaCapability }) {
  ...
  useEffect(() => {
    let active = true;
    void mediaApi.providers().then((next) => { if (active) setData(next); }).catch((cause: unknown) => { if (active) setError(cause instanceof Error ? cause.message : "读取媒体 Provider 失败"); });
    return () => { active = false; };
  }, []);
  ...
  useEffect(() => {
    ...
    setModelId(selected?.settings?.modelId ?? selected?.defaultModel ?? selected?.models[0]?.id ?? "");
  ...
  async function save() {
    ...
    try {
      ...
      setApiKey("");
      setData((current) => current ? { ...current, [capability]: current[capability].map((item) => item.id === selected.id ? { ...item, configured: true, baseUrl: result.baseUrl, settings } : item) } : current);
  ...
  async function remove() {
    ...
    try {
      await mediaApi.removeCredential(capability, selected.id);
      setData((current) => current ? { ...current, [capability]: current[capability].map((item) => item.id === selected.id ? { ...item, configured: false } : item) } : current);
      setMessage("凭据已移除");
  ...
  return <div className={styles.providerWorkspace}>
    <aside className={styles.providerRail} aria-label={`${labels[capability]} Provider`}>
      ...
      <div className={styles.providerList}>
        ...
        {!providers.length && <p className={styles.emptySettings}>正在读取 Provider…</p>}
      </div>
    </aside>
    <section className={styles.providerDetail} aria-live="polite">
      {selected ? <>
          ...
          <span className={selected.configured ? styles.settingsStatus : styles.settingsStatusIdle}><span />{selected.configured ? "已配置" : "未配置"}</span>
        </div>
        ...
        <section className={styles.mediaConfigSection} aria-labelledby="media-credentials-title">
          ...
          <div className={styles.mediaConfigGrid}>
            <label className={styles.settingsField}><span>API Key</span><input type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder={selected.requiresApiKey ? (selected.configured ? "输入新密钥以替换" : "输入 API Key") : "无需密钥"} autoComplete="new-password" disabled={!selected.requiresApiKey} /></label>
