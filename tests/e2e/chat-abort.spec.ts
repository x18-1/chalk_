...
const fixturePrefix = 'E2E Abort Fixture';
...
function createHangingFixtureProviderServer() {
...
async function restoreAbortWorkspace(page: Page, session: Partial<AbortSession>) {
...
async function openStreamingAbort(page: Page, title: string, session: Partial<AbortSession>) {
  ...
  await page.getByLabel('消息内容').fill('验证中止链路');
  await page.getByRole('button', { name: '发送消息' }).click();
  await expect(page.getByRole('button', { name: '停止', exact: true })).toBeVisible();
  await expect(page.getByText('先把已知条件写下来，')).toBeVisible();
}

async function withAbortFixture(page: Page, run: () => Promise<void>) {
  const providerServer = createHangingFixtureProviderServer();
...
test('student can abort an in-flight reply from the browser', async ({ page }) => {
  const session: Partial<AbortSession> = {};
  await withAbortFixture(page, async () => {
    try {
      await openStreamingAbort(page, '浏览器中止回答', session);
      await page.getByRole('button', { name: '停止', exact: true }).click();
      await expect(page.getByText('已停止', { exact: true })).toBeVisible();
      await expect(page.getByRole('button', { name: '停止', exact: true })).toHaveCount(0);
      await expect(page.getByRole('button', { name: '发送消息' })).toBeVisible();
      await expect(page.getByText('完整答案不应该出现。')).toHaveCount(0);
      ...
      await restoreAbortWorkspace(page, session);
    }
  });
});
